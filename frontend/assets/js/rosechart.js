...
Chart.rose = function() {
	...
	function createWedges( data ) {
		...
		wedges.append('svg:title')
			.text( function(d) { return d.legend + ': ' + Math.floor(Math.pow(d.radius,2) * Math.PI / numWedges) + ' links'; });
...
Chart.legend = function( entries ) {
	...
	legend.container = d3.select('#history-rose').append('div')
		.attr('class', 'legend');
...
Chart.slider = function( minVal, maxVal, step ) {
	...
	d3.select('#history-rose').append('input')
		.attr('class', 'slider')
		.attr('type', 'range')
		.attr('name', 'slider')
		.attr('min', minVal)
